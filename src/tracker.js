 ...
 * (see gen_canvas.py): 4 tags at fixed relative positions around a
 * postcard-sized frame. Ordinarily every layout is learnt from scratch by
 * sweeping the camera, because tags can be stuck anywhere - but this one
 * specific arrangement is known in advance, so two of its tags seen together
 * in a single frame is enough to anchor all four instantly, no sweep needed.
 * That matters most exactly where sweeping is hardest: a fixed desktop
 * webcam that can never move relative to the page.
 *
 * These entries were not hand-derived - hand-deriving the rotation/scale
 * convention risked a subtle, silent mismatch with however aruco.js actually
 * assigns a marker's corner order. Instead they were measured by running the
 * real detector + MarkerMap on an actual recording of this printed sheet and
 * reading back the converged map (see test/replay.py) - so whatever
 * convention the code uses, this matches it by construction.
 *
 * IMPORTANT: tags 0-3 are also the first four "tag N" stickers on the
...
class MarkerMap {
  ...
  /**
   * Called only when the map is still empty. If 2+ of a known preset's tags
